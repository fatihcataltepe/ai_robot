...
public class Tester {
   ...
   /**
    * Creates a new Rectangle2D that is grown by delta in each direction compared to the given Rectangle2D.
    *
    * @param rect
   ...
   public static Rectangle2D grow(Rectangle2D rect, double delta) {
      return new Rectangle2D.Double(rect.getX() - delta,
                                    rect.getY() - delta,
   ...
   public boolean testValidSteps(int testNo, boolean verbose) {
      ...
      if (!badSteps.isEmpty()) {
         System.out.println(String.format("FAILED: Step size limit exceeded for %d of %d step(s).", badSteps.size(), ps.getPath()
                                                                                                                       .size() - 1));
         if (verbose) {
   ...
   public boolean testJointAngles(int testNo, boolean verbose) {
      ...
      if (!badStates.isEmpty()) {
         System.out.println(String.format("FAILED: Invalid joint angle for %d of %d state(s).", badStates.size(), ps.getPath()
                                                                                                                    .size()));
         if (verbose) {
            if (verbose) {
   ...
   public boolean testSelfCollision(int testNo, boolean verbose) {
      ...
      if (!badStates.isEmpty()) {
         System.out.println(String.format("FAILED: Self collision for %d of %d state(s).", badStates.size(), ps.getPath().size()));
         if (verbose) {
            if (verbose) {
               System.out.println("Line for each invalid cfg:");
               System.out.println(addToAll(badStates, 2));
   ...
   /**
    * Checks if a configuration collides with itself. Uses a naive method where each link is checked for intersection with other
    * links.
    * 
    * @param cfg
   ...
   public boolean testBounds(int testNo, boolean verbose) {
      ...
      if (!badStates.isEmpty()) {
         System.out.println(String.format("FAILED: %d of %d" + " state(s) go out of the workspace bounds.",
                                          badStates.size(),
                                          ps.getPath().size()));
         if (verbose) {
            System.out.println("Line for each invalid cfg:");
   ...
   public boolean testCollisions(int testNo, boolean verbose) {
      ...
      if (!badStates.isEmpty()) {
         System.out.println(String.format("FAILED: %d of %d state(s) collide with obstacles.", badStates.size(), ps.getPath()
                                                                                                                   .size()));
         if (verbose) {
            System.out.println("Line for each invalid cfg:");
            System.out.println(addToAll(badStates, 2));
   ...
   /**
    * Returns whether the given config collides with any of the given obstacles.
    *
    * @param cfg
    *           the configuration to test.
    * @param obstacles
    *           the obstacles to test against.
    * @return whether the given config collides with any of the given obstacles.
    */
   ...
   public static void main(String[] args) {
      ...
      if (problemPath == null) {
         System.out.println("Usage: tester [-e maxError] [-v] " + "problem-file [solution-file]");
         System.exit(1);
      }
      ...
      if (solutionPath != null) {
         testsToRun.addAll(Arrays.asList(new String[]{"initial", "goal", "steps"}));
      }
      testsToRun.addAll(Arrays.asList(new String[]{"angles", "self-collision", "bounds", "collisions"}));
      int testNo = 1;
      int numFailures = 0;
